// ============================================================
// TALIX v3 — UI Components (updated for real data)
// ============================================================

function TAvatar({ name, color, size = 36, photo }) {
  if (photo) return <img src={photo} alt={name} style={{ width:size, height:size, borderRadius:'50%', objectFit:'cover', flexShrink:0 }} />;
  const initials = (name||'U').split(' ').map(w=>w[0]).join('').slice(0,2).toUpperCase();
  return (
    <div style={{ width:size, height:size, borderRadius:'50%', background:color||'#1A7A50', display:'flex', alignItems:'center', justifyContent:'center', color:'#fff', fontWeight:700, fontSize:size*0.36, flexShrink:0, fontFamily:'Poppins, sans-serif', letterSpacing:'-0.5px' }}>{initials}</div>
  );
}

function TCategoryBadge({ category }) {
...
function TConditionDot({ condition }) {
  ...
  return (
    <span style={{ display:'flex', alignItems:'center', gap:4, fontSize:12, color:'#666' }}>
      <span style={{ width:7, height:7, borderRadius:'50%', background:map[condition]||'#ccc', display:'inline-block' }} />{condition}
    </span>
  );
}
...
function TItemCard({ item, onClick, onUserClick, theme, currentUserId }) {
  const [liked, setLiked] = React.useState((item.likes||[]).includes(currentUserId));
  ...
  return (
    <div onClick={onClick} style={{ background:'#fff', borderRadius:20, overflow:'hidden', boxShadow:'0 2px 12px rgba(0,0,0,0.07)', cursor:'pointer', transition:'transform 0.18s, box-shadow 0.18s', border:'1px solid rgba(0,0,0,0.05)' }}
    ...
    >
      <div style={{ height:148, background:item.bgColor||'#F4F6F0', display:'flex', alignItems:'center', justifyContent:'center', position:'relative', overflow:'hidden' }}>
        {item.photo ? <img src={item.photo} alt={item.title} style={{ width:'100%', height:'100%', objectFit:'cover' }} /> : <span style={{ fontSize:52 }}>{categoryEmoji(item.category)}</span>}
...
function TTopBar({ title, subtitle, theme, rightEl }) {
  return (
...
function TInput({ label, type='text', placeholder, value, onChange, icon, required }) {
  return (
    <div style={{ marginBottom:16 }}>
      ...
      <div style={{ position:'relative' }}>
        ...
        <input type={type} placeholder={placeholder} value={value} onChange={onChange} required={required} style={{ width:'100%', padding:icon?'12px 16px 12px 42px':'12px 16px', border:'1.5px solid #E8EDE8', borderRadius:12, fontSize:14, fontFamily:'Poppins, sans-serif', outline:'none', boxSizing:'border-box', background:'#FAFBFA', color:'#1C2B2B', transition:'border-color 0.15s' }}
          onFocus={e=>e.target.style.borderColor='#1A7A50'} onBlur={e=>e.target.style.borderColor='#E8EDE8'}
        />
      </div>
    </div>
  );
...
function TButton({ children, onClick, variant='primary', theme, style:extra, disabled, type='button' }) {
  const styles = {
  ...
  return (
    <button type={type} onClick={onClick} disabled={disabled} style={{ padding:'12px 24px', borderRadius:100, border:'none', cursor:disabled?'not-allowed':'pointer', fontFamily:'Poppins, sans-serif', fontWeight:600, fontSize:14, transition:'opacity 0.15s, transform 0.15s', opacity:disabled?0.5:1, ...styles[variant], ...extra }}
...
function TModal({ title, onClose, children, width=480 }) {
  return (
...
function TToast({ message, type='success', onClose }) {
  React.useEffect(() => { const t = setTimeout(onClose, 3500); return ()=>clearTimeout(t); }, []);
  const colors = { success:{ bg:'#E8F5E9', border:'#4CAF50', color:'#2E7D32', icon:'✅' }, error:{ bg:'#FFEBEE', border:'#E53935', color:'#C62828', icon:'❌' }, info:{ bg:'#E3F2FD', border:'#1565C0', color:'#1565C0', icon:'ℹ️' } };
...
function NotificationBell({ userId, theme }) {
  ...
  return (
    <div style={{ position:'relative' }}>
      <button onClick={()=>{ setOpen(!open); if(!open) NotificationsService.markRead(userId); }} style={{ background:'#F4F6F0', border:'none', width:40, height:40, borderRadius:'50%', cursor:'pointer', fontSize:18, position:'relative', display:'flex', alignItems:'center', justifyContent:'center' }}>
        🔔
...
Object.assign(window, { TAvatar, TCategoryBadge, TConditionDot, TItemCard, TTopBar, TInput, TButton, TModal, TToast, NotificationBell, categoryEmoji });
