// ============================================================
// TALIX v3 — Admin Panel (real data from localStorage DB)
// ============================================================

const ADMIN_CREDS = { email:'admin@talix.usil.edu.pe', password:'admin2025' };

function AdminLoginPage({ onAdminLogin, onBack }) {
      ...
      <div style={{ width:420 }}>
        ...
        <div style={{ background:'#1A2332', borderRadius:24, padding:'32px 36px', border:'1px solid #2A3444' }}>
          ...
          <div style={{ marginBottom:14 }}>
            ...
            <input value={email} onChange={e=>setEmail(e.target.value)} placeholder="admin@talix.usil.edu.pe" type="email"
              ...
              onFocus={e=>e.target.style.borderColor='#F5A623'} onBlur={e=>e.target.style.borderColor='#2A3444'}
            />
          </div>
          <div style={{ marginBottom:20 }}>
            ...
            <input value={pass} onChange={e=>setPass(e.target.value)} onKeyDown={e=>e.key==='Enter'&&handle()} placeholder="••••••••" type="password"
              style={{ width:'100%', padding:'12px 16px', background:'#0F1923', border:'1.5px solid #2A3444', borderRadius:12, color:'#fff', fontFamily:'Poppins', fontSize:14, outline:'none', boxSizing:'border-box' }}
              onFocus={e=>e.target.style.borderColor='#F5A623'} onBlur={e=>e.target.style.borderColor='#2A3444'}
            />
          </div>
          {error&&<div style={{ background:'#2D1515', border:'1px solid #5C2020', borderRadius:10, padding:'10px 14px', marginBottom:16, fontSize:13, color:'#FF7070' }}>⚠️ {error}</div>}
          <button onClick={handle} disabled={loading} style={{ width:'100%', padding:13, borderRadius:100, border:'none', background:'#F5A623', color:'#1C2B2B', fontFamily:'Poppins', fontWeight:700, fontSize:15, cursor:loading?'not-allowed':'pointer', opacity:loading?0.7:1 }}>
            {loading?'⏳ Verificando...':'Ingresar al panel'}
          </button>
          <div style={{ textAlign:'center', marginTop:18 }}>
            <span onClick={onBack} style={{ fontSize:13, color:'#555', cursor:'pointer' }} onMouseEnter={e=>e.target.style.color='#888'} onMouseLeave={e=>e.target.style.color='#555'}>← Volver al acceso de alumnos</span>
          </div>
...
function AdminDashboard({ onLogout }) {
  ...
  const [stats, setStats] = React.useState({ users:[], items:[], convos:[], totalSwaps:0, totalCO2:'0', activeUsers:0 });
  const [search, setSearch] = React.useState('');
  const [statusFilter, setStatusFilter] = React.useState('todos');
  const [refreshKey, setRefreshKey] = React.useState(0);

  React.useEffect(() => {
    const interval = setInterval(()=>{ setStats(AdminService.getStats()); setRefreshKey(k=>k+1); }, 2000);
  ...
  const statusColor={ activo:'#4CAF50', inactivo:'#999', pendiente:'#F5A623', baneado:'#E53935' };
  const statusBg   ={ activo:'#E8F5E9', inactivo:'#F5F5F5', pendiente:'#FFF3E0', baneado:'#FFEBEE' };

  const filteredUsers = stats.users.filter(u =>
    (statusFilter==='todos'||u.status===statusFilter) &&
    ((u.displayName||'').toLowerCase().includes(search.toLowerCase())||(u.email||'').toLowerCase().includes(search.toLowerCase())||(u.faculty||'').toLowerCase().includes(search.toLowerCase()))
  );
  ...
  const exportEmails = () => {
    ...
    const a = document.createElement('a'); a.href=URL.createObjectURL(blob); a.download='talix-emails.txt'; a.click();
  };
    ...
    <div style={{ display:'flex', height:'100vh', fontFamily:'Poppins, sans-serif', background:'#0F1923' }}>
      ...
      <div style={{ marginLeft:240, flex:1, overflowY:'auto', height:'100vh' }}>
        <div style={{ padding:'20px 32px', background:'#1A2332', borderBottom:'1px solid rgba(255,255,255,0.07)', display:'flex', alignItems:'center', justifyContent:'space-between', position:'sticky', top:0, zIndex:50 }}>
          <div>
            <div style={{ fontWeight:700, fontSize:18, color:'#fff' }}>{navItems.find(n=>n.key===section)?.icon} {navItems.find(n=>n.key===section)?.label?.split(' (')[0]}</div>
          ...
          <div style={{ display:'flex', alignItems:'center', gap:10 }}>
            ...
            <button onClick={()=>PDFReportService.openPDF()} style={{ display:'flex', alignItems:'center', gap:6, background:'rgba(245,166,35,0.15)', border:'1px solid rgba(245,166,35,0.3)', padding:'8px 16px', borderRadius:100, cursor:'pointer', fontFamily:'Poppins', fontWeight:600, fontSize:12, color:'#F5A623', transition:'all 0.2s' }}
              onMouseEnter={e=>{e.currentTarget.style.background='rgba(245,166,35,0.25)';}} onMouseLeave={e=>{e.currentTarget.style.background='rgba(245,166,35,0.15)';}}
            >📄 Exportar PDF</button>
        ...
        <div style={{ padding:'28px 32px' }}>
            ...
            <div>
              ...
              <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:20 }}>
                <div style={{ background:'#1A2332', borderRadius:20, padding:24, border:'1px solid rgba(255,255,255,0.07)' }}>
                  ...
                  {faculties.length===0?<div style={{ color:'#444', fontSize:13 }}>Sin usuarios aún</div>:faculties.map(([f,count],i)=>{
                ...
                <div style={{ background:'#1A2332', borderRadius:20, padding:24, border:'1px solid rgba(255,255,255,0.07)' }}>
                  ...
                  {recentActivity.length===0?<div style={{ color:'#444', fontSize:13 }}>Sin actividad aún</div>:recentActivity.map((a,i)=>(
            ...
            <div>
              <div style={{ display:'flex', gap:12, marginBottom:18, alignItems:'center', flexWrap:'wrap' }}>
                <div style={{ position:'relative', flex:1, minWidth:200 }}>
                  ...
                  <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Buscar por nombre, correo o facultad..."
              ...
              <div style={{ background:'#1A2332', borderRadius:20, overflow:'hidden', border:'1px solid rgba(255,255,255,0.07)' }}>
                ...
                {filteredUsers.length===0?(
                  ...
                  <div key={u.id} style={{ display:'grid', gridTemplateColumns:'2fr 2.5fr 1.5fr 0.8fr 1fr 1fr', padding:'14px 20px', borderBottom:i<filteredUsers.length-1?'1px solid rgba(255,255,255,0.04)':'none', alignItems:'center' }}>
                    ...
                    <div>
                      <span style={{ background:statusBg[u.status||'activo']||'#E8F5E9', color:statusColor[u.status||'activo']||'#4CAF50', padding:'4px 10px', borderRadius:100, fontSize:11, fontWeight:600 }}>
                        {u.status||'activo'}
                      </span>
                    </div>
                    <div style={{ display:'flex', gap:6 }}>
                      {u.status!=='baneado'&&(
                        <button onClick={()=>handleBan(u.id)} style={{ padding:'5px 10px', background:'rgba(229,57,53,0.12)', border:'1px solid rgba(229,57,53,0.25)', borderRadius:8, color:'#E57373', fontSize:11, cursor:'pointer', fontFamily:'Poppins', fontWeight:600 }}>Banear</button>
                      )}
                    </div>
                  </div>
                ))}
          ...
          {section==='items'&&(
            <div style={{ background:'#1A2332', borderRadius:20, overflow:'hidden', border:'1px solid rgba(255,255,255,0.07)' }}>
              <div style={{ padding:'16px 20px', borderBottom:'1px solid rgba(255,255,255,0.07)', fontWeight:700, fontSize:15, color:'#fff' }}>📦 Artículos en plataforma ({stats.items.length})</div>
              {stats.items.length===0?(
                ...
                <div key={item.id} style={{ display:'flex', alignItems:'center', gap:14, padding:'14px 20px', borderBottom:i<stats.items.length-1?'1px solid rgba(255,255,255,0.04)':'none' }}>
                  ...
                  <button onClick={()=>handleDeleteItem(item.id)} style={{ padding:'5px 12px', background:'rgba(229,57,53,0.12)', border:'1px solid rgba(229,57,53,0.25)', borderRadius:8, color:'#E57373', fontSize:11, cursor:'pointer', fontFamily:'Poppins', fontWeight:600, flexShrink:0 }}>Retirar</button>
                </div>
              ))}
            </div>
          )}
          ...
          {section==='chat'&&(
            <div style={{ background:'#1A2332', borderRadius:20, overflow:'hidden', border:'1px solid rgba(255,255,255,0.07)' }}>
              <div style={{ padding:'16px 20px', borderBottom:'1px solid rgba(255,255,255,0.07)', fontWeight:700, fontSize:15, color:'#fff' }}>💬 Conversaciones activas ({stats.convos.length})</div>
...
Object.assign(window, { AdminLoginPage, AdminDashboard });
